...
test("hover item updates on keyUp or keyDown", async (t) => {
  ...
  await handleKeyboard(
    "ArrowDown",
    document.querySelector(".svelte-select-list"),
  );
...
test("blur should close list and remove focus from select", async (t) => {
  ...
  t.ok(
    document.querySelector(".svelte-select input") !== document.activeElement,
  );
  select.$destroy();
});
...
test("Select container styles can be overridden", async (t) => {
  ...
  t.equal(
    document.querySelector(".svelte-select").style.cssText,
    `padding-left: 40px;`,
  );
  select.$destroy();
});
...
test("inputStyles prop applies css to select input", async (t) => {
  ...
  t.equal(
    document.querySelector(".svelte-select input").style.cssText,
    `padding-left: 40px;`,
  );
  select.$destroy();
});
...
test("groups should be sorted by expression", async (t) => {
  ...
  t.ok(
    document.querySelector(".list-group-title").textContent.trim() === "Savory",
  );
  t.ok(
    document.querySelector(".list-item .group-item").textContent.trim() ===
      "Pizza",
  );
...
test("when loadOptions method is supplied, multiple is true and filterText has length then items should populate via promise resolve", async (t) => {
  ...
  t.ok(
    document
      .querySelector(".multi-item span")
      .innerHTML.startsWith("Juniper Wheat Beer"),
  );
  select.$destroy();
});
...
test("when items in list filter or update then first item in list should highlight", async (t) => {
  ...
  await handleKeyboard("ArrowDown");
  t.ok(
    document.querySelector(".svelte-select-list .hover").innerHTML ===
      "Chocolate",
  );
...
test("when multiple with items and value supplied as just strings then value should render correctly", async (t) => {
  ...
  t.ok(
    document.querySelector(".multi-item span").innerHTML.startsWith("Pizza"),
  );
...
test("When listAutoWidth is set to false list container should have style of width:auto", async (t) => {
  ...
  await wait(0);
  const listWidth = document.querySelectorAll(".svelte-select-list")[0].style
    .width;
  t.ok(listWidth === "auto");
  select.$destroy();
});
...
test("when loadOptions and items is supplied then list should close on blur", async (t) => {
  ...
  let loadOptions = async (filterText) => {
    const res = await fetch(
      `https://api.punkapi.com/v2/beers?beer_name=${filterText}`,
    );
...
test("when switching between multiple true/false ensure Select continues working", async (t) => {
  ...
  t.ok(
    JSON.stringify(select.value) ===
      JSON.stringify([{ value: "chips", label: "Chips" }]),
  );
...
test("when named slot list show content", async (t) => {
  ...
  t.ok(
    document.querySelector(".svelte-select-list").innerHTML.trim() === "onetwo",
  );
...
test("when named slot input-hidden", async (t) => {
  ...
  t.ok(
    document.querySelector('input[type="hidden"][name="test"]').value.trim() ===
      "one",
  );
...
test("when named slot item show content", async (t) => {
  ...
  t.ok(
    document.querySelector(".svelte-select-list .item").innerHTML === "* one *",
  );
...
test("when named slots list-prepend and list-append show content", async (t) => {
  ...
  t.ok(
    document
      .querySelector(".svelte-select-list")
      .innerHTML.startsWith("prepend"),
  );
  t.ok(
    document.querySelector(".svelte-select-list").innerHTML.endsWith("append"),
  );
...
test("when --multi-item-color css variable supplied then CSS should apply", async (t) => {
  ...
  t.ok(
    getComputedStyle(document.querySelector(".multi-item")).getPropertyValue(
      "color",
    ) === "rgb(255, 0, 0)",
  );
...
test("when loadOptions and value then it should set initial value", async (t) => {
  ...
  t.ok(
    document.querySelector(".value-container .selected-item").innerHTML ===
      "cake",
  );
  await wait(500);
  t.ok(
