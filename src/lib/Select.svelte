<script>
  ...
  function dispatchSelectedItem() {
    ...
    if (
      !prev_value ||
      JSON.stringify(value[itemId]) !== JSON.stringify(prev_value[itemId])
    ) {
      dispatch("input", value);
    }
  }
    ...
    switch (e.key) {
      ...
      case "Backspace":
        ...
        if (multiple && value && value.length > 0) {
          ...
          activeValue =
            value.length > activeValue ? activeValue - 1 : undefined;
        }
  ...
  function isItemSelectable(item) {
    return (
      (item.groupHeader && item.selectable) ||
      item.selectable ||
  ...
  const [floatingRef, floatingContent, floatingUpdate] =
    createFloatingActions(_floatingConfig);
      ...
      {:else if filteredItems.length > 0}
        {#each filteredItems as item, i}
          ...
          >
            <div
              use:activeScroll={{
                scroll: isItemActive(item, value, itemId),
                listDom,
              }}
